...
- `test_support.rs` -- `#[doc(hidden)]` helpers for the integration tests. `wait_for_watcher_ready` (call immediately before a trigger) and `watcher_barrier` (call after a trigger, before asserting an event was *not* emitted) both drive one probe round trip; `OS_EVENT_TIMEOUT` bounds how long a live watch may take to report a change.
