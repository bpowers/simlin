...
- `discovery.rs`, `scan.rs` -- Recursive walk via `ignore` (respects `.gitignore`, excludes `node_modules` and `target`); `format_for_path` decides which paths register as projects, and is the crate's single extension dispatcher -- the walker, the watcher's event classifier, and the HTTP read/save handlers all call it rather than re-deriving the mapping. `scan::legacy_sidecar_pairs` lists same-stem `.mdl`/`.sd.json` pairs (the on-disk trace of an earlier release that saved `.mdl` edits into a JSON "sidecar") so `main.rs` can warn once at startup; both files are ordinary independent projects and nothing else treats the pair specially.
...
- `writer.rs` -- `resolve_save_target` (pure dispatch) + `serialize_project` (render bytes) + `commit_write` (atomic file I/O). Every format overwrites its own file in place: `.stmx`/`.xmile` as regenerated XMILE, `.mdl` as regenerated Vensim text (`to_mdl_with_warnings`, sketch included), `.sd.json` as pretty JSON. `WriteOutcome::warnings` carries the MDL writer's lossiness warnings (converted through `simlin_mcp_core::types::mdl_export_warnings_to_outputs` so serve, stdio MCP, and pysimlin report the same thing); the writer's hard errors (a project Vensim structurally cannot hold) surface as `SaveDiskError::MdlSerialize`. `preflight_serialize` runs the writer and discards the bytes; both save paths (HTTP `save_project`, MCP `RegistryAccess::save`) call it on the validated incoming project BEFORE `check_increment_and_merge` so an unrepresentable model is a 422 / `AccessError::Validation` while the doc is still untouched -- merging first would leave the entry at a version whose state can never reach disk.
- `watcher.rs` -- `WatcherActor` consumes `notify-debouncer-full` batches via a tokio mpsc channel. Per-event handlers: `handle_model_change` (read, hash-compare for echo suppression, parse, validate, merge, broadcast), `handle_model_removal` (drop registry entry, broadcast `ProjectRemoved`), `handle_model_rename` (preserves the `LoroDoc`, broadcasts `ProjectRenamed`), `handle_git_change` (invalidate `GitProbe` cache, re-evaluate every entry in the affected repo). `spawn_watcher` returns a `WatcherHandle`, which carries the actor's `JoinHandle` and a `probe_sightings()` `watch` channel -- see the readiness contract below.
- `test_support.rs` -- `#[doc(hidden)]` helpers for the integration tests, gated behind the `test-support` feature so they never reach the shipped binary; `tests/` gets them via the self dev-dependency in `Cargo.toml` that enables the feature. `wait_for_watcher_ready` (call immediately before a trigger) and `watcher_barrier` (call after a trigger, before asserting an event was *not* emitted) both drive one probe round trip; `OS_EVENT_TIMEOUT` bounds how long a live watch may take to report a change.
- `handlers.rs` -- The Axum HTTP handlers (`list_projects`, `get_project`, `save_project`, `create_new_project`, `updates_ws_handler`) plus the `AppState`. The save handler runs validation, pre-flights the writer (`writer::preflight_serialize`; an unrepresentable model is a 422 before the doc is touched), calls `check_increment_and_merge`, calls `commit_write`, refreshes registry mtime/size, publishes `WsMessage::ProjectChanged { source: User }`, and returns `SaveResponse { version, warnings }` -- `warnings` (elided when empty) is the writer's lossiness list, which the SPA shows as a save notice.
...
- `EditorHost.tsx` (in the `components` subdirectory) -- Hosts `@simlin/diagram`'s `<Editor>`. Wires real `onSave` (409 conflict refetch, 422 validation toast, and the save response's `warnings` rendered as a persistent save notice until the next clean save), debounced `onSelectionChanged` (150ms), and `projectFocused` on mount/path change.
...
- **Path resolution goes through `path_resolution`** -- the canonicalize-within-root check and the resolve-create-target check are implemented once in `src/path_resolution.rs` and called by every consumer (HTTP handlers, MCP `RegistryAccess`, watcher). Inlined reimplementations are a maintenance bug-source we have repeatedly paid for; new consumers must call the existing helpers or extend the module rather than open-coding the rule.
...
- **`spawn_watcher` returning does not mean the watch is live** -- `Watcher::watch` hands the stream to the OS and returns; on macOS `fseventsd` completes registration asynchronously, and every change made before it does is lost permanently, not delayed (the stream's "since now" event id is resolved at registration, not at `watch()`). Worse, a stream that *is* live still drops the next event it should report after a couple of seconds of idle, so readiness is not a one-shot property -- a caller must fire its trigger immediately after establishing it, with no sleep in between. Both effects reproduce against raw `notify` with no debouncer; see the measurements in `PROBE_FILE_PREFIX`'s rustdoc, which are from one host with a saturated `fseventsd` and are a worst case rather than typical. Nothing in `notify` exposes a "stream started" callback, and it does not expose `FSEventStreamFlushSync`, the one Apple primitive that would answer this. The only sound readiness check is to cause a filesystem event and wait for it to come back: create a file named `{PROBE_FILE_PREFIX}{nonce}` and wait for `WatcherHandle::probe_sightings()` to reach `nonce`. Probe files carry no model extension, so they classify as `Ignored` and never touch the registry or the event bus; the watcher reports the sighting and still classifies the event, so an event naming both a probe and a real model is not dropped. **Never `sleep` to wait for a watch to become live** -- there is no bound to sleep for.
...
- **Every format is read from and written to its own file** -- a `.mdl` is rewritten in place as Vensim text exactly as a `.stmx` is rewritten as XMILE; the file the request names is the project. Never reintroduce a per-format alias that maps one path onto another (the earlier `.mdl` -> `.sd.json` "sidecar" rule): every consumer -- HTTP get/save, MCP open/save, the watcher's classifier and echo suppression -- had to re-apply it, and each one that forgot was a bug. Constructs the on-disk format cannot express are a warnings channel (`WriteOutcome::warnings` -> `SaveResponse.warnings` / `SaveOutcome::warnings`), never a reason to write somewhere else.
...
- `simlin-mcp-core` -- the `ProjectAccess` trait (`save` returns `SaveOutcome { version, warnings }`), the three reused tools, output types, error shape, and `mdl_export_warnings_to_outputs`.
