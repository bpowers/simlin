#!/usr/bin/env bash
# Generate platform-specific npm package.json files for @simlin/serve distribution.
#
# Output goes to npm/@simlin/serve-<platform>/ relative to this script. CI copies
...
for entry in "${PLATFORMS[@]}"; do
  ...
  pkg_name="@simlin/serve-${platform}"
  ...
  cat > "$pkg_dir/package.json" <<JSON
{
  "name": "$pkg_name",
  "version": "$VERSION",
  "description": "Platform binary for @simlin/serve ($platform)",
  "os": ["$os"],
  "cpu": ["$cpu"],
  "files": ["bin"],
  "license": "Apache-2.0",
