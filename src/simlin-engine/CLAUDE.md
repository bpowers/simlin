...
- **`src/db_analysis.rs`** - Salsa-tracked causal graph analysis: `model_causal_edges`, `model_loop_circuits`, `model_cycle_partitions`, `model_detected_loops`. Element-level variants `model_element_loop_circuits` and `model_element_cycle_partitions` operate on the element-level graph from `model_element_causal_edges`. Produces `DetectedLoop` structs with polarity.
