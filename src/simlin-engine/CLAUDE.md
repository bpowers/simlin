...
- **`src/ltm_augment.rs`** - Equation generators for LTM synthetic variables: `generate_link_score_equation_for_link` (ceteris-paribus link scores), `generate_loop_score_variables` (emits one `loop_score` per loop; relative loop scores are computed post-simulation in `ltm_post.rs`), `build_partial_equation` (AST-based PREVIOUS wrapping), `quote_ident` (identifier quoting for equations). Array support: `classify_reducer` (walks target Expr2 AST to identify reducing builtins -- Linear for SUM/MEAN, Nonlinear for MIN/MAX/STDDEV/RANK, Constant for SIZE), `generate_element_to_scalar_equation` (per-element link score equations for arrayed-to-scalar edges). Module link score formulas (black-box delta-ratio and composite-ref) are inlined directly into `link_score_equation_text` in `db.rs`.
