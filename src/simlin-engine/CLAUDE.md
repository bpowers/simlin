...
- **`src/ltm_augment.rs`** - Equation generators for LTM synthetic variables: `generate_link_score_equation_for_link` (ceteris-paribus link scores), `generate_loop_score_variables` (loop/relative loop scores), `generate_module_link_score_equation` (black-box module transfer), `generate_module_input_link_score_equation` (composite-ref module inputs), `build_partial_equation` (AST-based PREVIOUS wrapping)
