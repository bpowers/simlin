...
- **`src/ltm.rs`** - Loops That Matter: core data types and causal graph construction. `CausalGraph` builds adjacency lists and detects feedback loops via Johnson's circuit algorithm. `Loop` struct carries `id`, `links`, `stocks`, `polarity`, and `dimensions` (non-empty for A2A loops where per-element evaluation is needed; empty for scalar or mixed loops). `CyclePartitions` groups loops into stock-to-stock SCCs. `Link` records polarity between two variables and carries an optional `shape: Option<RefShape>` populated when loops are built in `db_ltm::build_element_level_loops` (None means fall back to `RefShape::Bare` at link-score emission). `LoopPolarity` is determined by counting negative links in a cycle.
