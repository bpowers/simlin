...
mod wildcard_tests {
    ...
    #[test]
    #[ignore]
    fn wildcard_in_multidim() {
        // Test wildcard in multi-dimensional arrays
        ArrayTestProject::new("wildcard_multidim")
            .indexed_dimension("Row", 2)
            .indexed_dimension("Col", 3)
            .array_const("matrix[Row,Col]", 5.0)
            .array_aux("row_slice[Col]", "matrix[Row.1, *]")
            .assert_compile_error(ErrorCode::TodoWildcard);
    }
