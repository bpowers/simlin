...
fn wildcard_reduce_over_apply_to_all_survives_re_rendering() {
    ...
    // A genuinely per-element arrayed variable, so the second render also
    // exercises the `Arrayed` branch of the wildcard recovery.
    let per_element = Variable::Aux(datamodel::Aux {
        ident: "per_element".to_owned(),
        equation: Equation::Arrayed(
            vec!["DimA".to_owned()],
            vec![
    ...
    let target2 = make_aux("target2", "sum(per_element[*])", None, "");
    let mut project = make_project(vec![make_model(vec![arr, per_element, target, target2])]);
    ...
    // The re-import keeps the apply-to-all as ONE equation (the importer no
    // longer explodes a single A2A equation into per-element slots). What the
    // wildcard recovery actually needs is the variable's declared DIMENSIONS,
    // which both equation shapes carry -- `per_element` below is the same
    // round trip over a genuinely per-element variable, so the `Arrayed` path
    // through `WriterContext` stays covered.
    assert!(
        matches!(arr2.get_equation(), Some(Equation::ApplyToAll(..))),
        "a single apply-to-all equation must re-import as one equation: {:?}",
        arr2.get_equation(),
    );
    let per_element = p2.models[0]
        .variables
        .iter()
        .find(|v| v.get_ident() == "per_element")
        .expect("per_element survives the round trip");
    assert!(
        matches!(per_element.get_equation(), Some(Equation::Arrayed(..))),
        "genuinely per-element equations must stay per-element: {:?}",
        per_element.get_equation(),
    );
    ...
    assert!(
        // The writer spells the MDL name with a space (`quoted_space_to_underbar`
        // reversed), so match the emitted spelling rather than the ident.
        mdl2.contains("SUM(per element[DimA!])"),
        "second render lost the bang form over a per-element array:\n{mdl2}"
    );
