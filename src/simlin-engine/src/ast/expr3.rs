...
/// fragments (`TempStrategy::Recycle`). Every array materialized after
/// subscript resolution draws from the fragment's one allocator, which makes
/// ids dense without a reconciliation pass.
///
/// Two scoping operations refine the plain counter:
///
/// * [`Self::element_scopes`]: an apply-to-all or arrayed equation evaluates
///   its elements one after another, and each element's
...
/// moves the allocator past every id any element used. The array-operand
/// materializer owns the guard while it inserts each element's temps, so no
/// later pass needs to reconcile or renumber them.
#[must_use]
