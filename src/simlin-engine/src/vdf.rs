...
mod tests {
    ...
    mod file_io_tests {
        ...
        fn try_vdf_file(path: &str) -> Option<VdfFile> {
        ...
        fn test_slot_candidate_counts_by_gap() {
            ...
            let pop6 = match try_vdf_file(
                "../../third_party/uib_sd/fall_2008/sd202/assignments/assignment_6/Current.vdf",
            ) {
                Some(v) => v,
                None => {
                    eprintln!("skipping: uib_sd test data not available");
                    return;
                }
            };
        ...
        fn test_section6_ref_stream_counts() {
            ...
            if let Some(zambaqui) = try_vdf_file("../../third_party/uib_sd/zambaqui/baserun.vdf") {
        ...
        fn test_section5_dimension_sizes_match_zambaqui_model_dims() {
            let vdf = match try_vdf_file("../../third_party/uib_sd/zambaqui/baserun.vdf") {
                Some(v) => v,
                None => {
                    eprintln!("skipping: uib_sd test data not available");
                    return;
        ...
        fn test_record_ot_ranges_partition_selected_files() {
            ...
            let econ = vdf_file("../../test/bobby/vdf/econ/base.vdf");
            let wrld3 = vdf_file("../../test/metasd/WRLD3-03/SCEN01.VDF");
            let zambaqui = try_vdf_file("../../third_party/uib_sd/zambaqui/baserun.vdf");

            let mut cases: Vec<(&str, &VdfFile, usize)> = vec![
                ("water", &water, 8usize),
                ("econ", &econ, 61usize),
                ("wrld3", &wrld3, 234usize),
            ];
            if let Some(ref z) = zambaqui {
                cases.push(("zambaqui", z, 306usize));
            }

            for (label, vdf, expected_ranges) in cases {
            ...
            if let Some(ref z) = zambaqui {
                let z_ranges = z.record_ot_ranges();
                assert!(
