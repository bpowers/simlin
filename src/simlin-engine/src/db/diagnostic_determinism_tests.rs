...
//! constraints were generated by iterating the model's variable map (a
...
/// scalar `SMTH1(vals[*], 3)` shape, whose hoisted helper holds an array
/// slice in a scalar equation): the implicit-helper probe loop in
...
fn implicit_helper_diagnostics_are_reproducible() {
    let datamodel = TestProject::new("implicit_repro")
        ...
        .aux("out_a", "SMTH1(vals[*], 3)", None)
        .aux("out_b", "SMTH1(vals[*], 5)", None)
        .flow("g", "(out_a + 1) * 0.01", None)
        .stock("s", "10", &["g"], &[], None)
        .build_datamodel();
