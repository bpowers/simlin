...
// for the fragment characterization/determinism tests, which drive one
// variable's compile directly rather than through a whole-model walk.
#[cfg(test)]
pub(crate) use compile::compile_ltm_synthetic_fragment;
pub(crate) use link_scores::ltm_partial_equation_warning;
#[cfg(test)]
...
// design), re-exported so every consumer derives rows from one function. Only
// the STRUCTURED form crosses this boundary: both `db::analysis` consumers --
// `emit_agg_routed_edges` (GH #783) and `emit_edges_for_reference`'s
// `PerElement` arm (GH #525) -- read the per-axis parts, because a canonical
// element name can contain a comma and a joined-then-split round-trip would
// mis-read one coordinate as two.
pub(crate) use loops::{ReadSliceRowParts, read_slice_row_parts};
// The joined projection crosses this boundary for TESTS only -- and only to
// build expected node NAMES (`db::element_graph_proptest`), never to be split
// back apart. Production's remaining users are the link-score emitters inside
// `db::ltm`, which reach `loops` directly and likewise want a name.
#[cfg(test)]
pub(crate) use loops::read_slice_rows;
...
///
/// Checks targets with a `DepLag::Previous` occurrence and no
/// `DepLag::Current` occurrence. An `Initial` fallback or sibling snapshot is
/// not an instantaneous edge and therefore does not cancel the lagged target.
/// A reference that appears both inside and outside `PREVIOUS(...)` keeps its
...
/// routing. Module calls have already been expanded in the source variable's
/// authoritative AST before LTM equations are generated, so qualified reads
/// of those modules resolve through `model_implicit_var_info`; LTM parsing does
/// not own a second module namespace. This structure collects the remaining
/// implicit helpers across all LTM equations in a model so that
/// `compute_layout` can allocate slots and `assemble_module` can compile them.
...
/// model.
/// This query intentionally derives no diagnostics. Auto-flip `Warning` facts
/// belong to `model_ltm_variables`' returned result and are emitted only by the
/// non-recursive per-model owner in `db::diagnostic`; a tracked query may be
/// read by multiple callers without multiplying its warnings.
/// `model_ltm_variables` sets its returned `mode` from this query, so the two
/// never drift.
...
pub fn model_ltm_mode(db: &dyn Db, model: SourceModel, project: SourceProject) -> super::LtmMode {
    ...
    if var_scc_size > scc_node_budget {
        return LtmMode::Discovery;
    }
    ...
    if tiered.truncated || tiered.slow_path_largest_scc > scc_node_budget {
        LtmMode::Discovery
    } else {
        LtmMode::Exhaustive
    }
...
pub fn model_ltm_variables(
    ...
    use super::{
        Diagnostic, DiagnosticSeverity, LtmSyntheticVar, LtmVariablesResult,
        causal_graph_from_edges, causal_graph_with_modules, generate_max_abs_selection,
        model_causal_edges, model_element_cycle_partitions, model_loop_circuits_tiered,
        model_pinned_loops, module_input_pathways_from_edges,
    };
    ...
    if let Some(rejection) = &ref_sites.site_width_rejection {
        ...
        diagnostics.push(
            Diagnostic::assembly(msg, DiagnosticSeverity::Warning)
                .with_context(model.name(db).clone(), Some(rejection.variable.clone())),
        );
        return LtmVariablesResult {
            vars: vec![],
            loop_partitions: indexmap::IndexMap::new(),
    ...
    let var_auto_flipped = !is_discovery_user && var_scc_size > scc_node_budget;
    ...
    if var_auto_flipped {
        let msg = format!(
            "LTM analysis auto-switched from exhaustive to discovery mode: \
             ...
             strategy.",
            var_scc_size, scc_node_budget,
        );
        diagnostics.push(
            Diagnostic::assembly(msg, DiagnosticSeverity::Warning)
                .with_context(model.name(db).clone(), None),
        );
    }
    ...
    let loops: Option<Vec<Loop>> = if !is_discovery {
        ...
        // discovery path.
        else if tiered.slow_path_largest_scc > scc_node_budget {
            let msg = format!(
                "LTM analysis auto-switched from exhaustive to discovery mode: \
                ...
                tiered.slow_path_largest_scc, scc_node_budget,
            );
            diagnostics.push(
                Diagnostic::assembly(msg, DiagnosticSeverity::Warning)
                    .with_context(model.name(db).clone(), None),
    ...
    // correspond -- see `unscoreable_conservative_edge_warning`).
    ...
    fn unresolved_loop_score_warning(db: &dyn Db, model: SourceModel, loop_id: &str) -> Diagnostic {
        Diagnostic::assembly(
            format!(
    ...
    if let Some(ref detected_loops) = loops {
        ...
        for l in detected_loops {
            ...
            if !emitted_loop_score_names.contains(&expected) {
                diagnostics.push(unresolved_loop_score_warning(db, model, l.id.as_str()));
            }
        }
    ...
    for (name, reason) in &pinned.invalid {
        ...
        let _ = name;
        diagnostics.push(
            Diagnostic::assembly(reason.clone(), DiagnosticSeverity::Warning)
                .with_context(model.name(db).clone(), None),
        );
    }
        ...
        for pin in &pinned.loops {
            ...
            for pin_loop in loops_to_emit {
                ...
                if pin_loop_vars.is_empty() {
                    diagnostics.push(unresolved_loop_score_warning(
                        db,
                        model,
                        pin_loop.id.as_str(),
                    ));
    ...
    // returned diagnostic facts -- so an un-gated warning here is thousands of
        ...
        for (idx, pathway_links) in port_pathways.iter().enumerate() {
            ...
            let link_score_refs: Vec<String> = pathway_links
                ...
                .map(|link| {
                    ...
                    {
                        diagnostics.push(unresolved_pathway_link_warning(
                            db,
                            model,
                            input_port.as_str(),
                            link.from.as_str(),
...
/// Build a `Warning` when a sub-model PATHWAY equation references a
...
fn unresolved_pathway_link_warning(
    db: &dyn Db,
    model: SourceModel,
    input_port: &str,
    from: &str,
    ...
    link_scores::ltm_edge_warning(db, model, None, from, to, msg)
}
