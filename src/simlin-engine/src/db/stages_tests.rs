...
/// registry and duplicate-ident errors from datamodel values while the query
/// reads salsa inputs and memoized groups. So this is a real cross-check, not
...
/// constructor's IMPLICIT build, which independently applies the stdlib
/// classification.
///
...
/// no such input exists: every input stage0 reads either changes its value (its
/// own variables' parses, name, macro spec) or is shared with `model_stage1`
/// (the dimensions context). A test built on one of those would measure the
/// invalidation lever rather than value equality.
///
