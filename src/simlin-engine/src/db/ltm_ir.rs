...
fn classify_iterated_dim_shape(
    ...
    // Two source axes projected from the same target dimension are occurrence
    // sensitive. `Bare` has only the variables' dimension lists and cannot
    // preserve which occurrence supplied each coordinate; `PerElement` keeps
    // the reference site's axis list. A target that itself repeats the name is
    // not representable by the name-keyed score layout, so the score emitter
    // declines it with its established warning. Keeping the semantic edge
    // derivation exact and refusing that score is preferable to emitting a
    // plausible score for a different projection.
    let all_iterated = axes.iter().all(|a| matches!(a, AxisRead::Iterated { .. }));
    if all_iterated {
        let mut seen = std::collections::HashSet::new();
        let repeats_a_target_dim = axes
            .iter()
            ...
            .any(|dim| !seen.insert(dim));
        if !repeats_a_target_dim {
