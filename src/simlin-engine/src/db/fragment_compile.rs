...
/// builtins) to symbolic bytecodes.
///
/// **Salsa-tracked, keyed on the helper's own canonical name.** It used not to
/// be, on the reasoning that "the parent variable's parse result already
/// provides salsa caching" -- which is true of the PARSE and of nothing else.
/// The lowering (`lower_implicit_var` -> `variable::parse_var` ->
/// `lower_variable`) and the per-phase codegen ran on every assembly, so a
/// model's helpers were recompiled from scratch each time `assemble_module`
/// re-ran. On C-LEARN that is 651 calls costing ~12% of a cold compile, and
/// ~28% of the cost of a WARM single-equation edit -- by far the largest share
/// of a recompile that should have touched one variable and its consumers.
///
...
/// whole `ModelDepGraphResult` the caller used to pass in, for the same reason
/// `compile_var_fragment` reads `var_runlist_membership`: a three-bit
/// projection backdates when this helper's membership is unchanged, where the
/// whole result re-executes every helper's fragment whenever any variable's
/// dependencies move.
#[salsa::tracked(returns(ref))]
