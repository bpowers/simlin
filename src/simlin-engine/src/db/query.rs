...
/// project's unit name/alias/equation maps. Its cost is flat in the model count,
/// but it is per project and permanent, so a future field on `Context` is paid
/// twice.
#[salsa::tracked(returns(ref))]
...
///     parse-synthesized. `db::model_scope` checks every spliced template and is
///     the tripwire if a template ever gains one.
...
///   - `db::model_scope::model_module_targets` -- projects the same metadata
///     into an iterative worklist, so a cycle terminates.
///
...
///
/// `db::model_scope::model_scope_models` walks the same edges plus the implicit
/// ones and is iterative, so it is unaffected either way.
#[salsa::tracked(returns(ref))]
