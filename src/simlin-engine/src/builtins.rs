...
/// run-invariance classifier (`compiler::invariance`). Dependency identity,
/// phase and lag come from `db::DepRef`, not a second compiler-IR walk.
#[cfg_attr(feature = "debug-derive", derive(Debug))]
#[derive(Clone, Copy, PartialEq, Eq)]
pub enum Invariance {
...
///
/// This is the authoritative name check used by walk-time module expansion.
/// Callers add their own structural logic on top (for example PREVIOUS
/// argument-count checks and INIT inclusion for A2A).
