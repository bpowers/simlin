...
fn test_section6_display_record_stream_shape() {
    ...
    assert!(water_records.is_empty(), "water should have no parsed display records");
    assert!(pop_records.is_empty(), "pop should have no parsed display records");
    assert_eq!(econ_records.len(), 4, "econ display-record count should be stable");
    assert_eq!(wrld3_records.len(), 55, "WRLD3 display-record count should be stable");

    for (label, vdf, records) in [("econ", &econ, &econ_records), ("wrld3", &wrld3, &wrld3_records)]
    {
        for rec in records {
            assert!(
            ...
            assert_eq!(rec.words[11], 1, "{label}: expected stable display-record flag");
