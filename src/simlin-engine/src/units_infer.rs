...
// each project model's per-variable lowered unit view.

/// Run inference over `model_name` with every project model's per-variable
/// lowered view in the map, the way `db::units::check_model_units` drives
...
fn infer_project(
    ...
    let models_s1: HashMap<Ident<Canonical>, UnitModel> = sync
        .project
        .models(&db)
        .values()
        .map(|src_model| {
            let model = crate::db::unit_model(&db, *src_model, sync.project);
