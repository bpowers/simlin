...
// * `model_scope` -- lightweight module topology for model-wide unit analysis.
...
mod macro_registry;
mod model_scope;
pub(crate) use model_scope::{model_scope_models, source_model_is_stdlib};
mod units;
#[cfg(test)]
pub(crate) use units::unit_model;
mod var_fragment;
...
impl SimlinDb {
    ...
    /// which is what `SimlinDb::default()` gives them too.
    pub(crate) fn with_storage(storage: salsa::Storage<Self>) -> Self {
        SimlinDb {
            storage,
            ..Default::default()
        }
...
pub(super) fn find_model_output_ports_for_module(
    ...
    if lowered_variable_by_name(db, model, project, module_var_name).is_some_and(|variable| {
        matches!(
            variable.kind,
