...
It records the **current layout behavior**, scored with the committed calibrated
`MetricWeights::default()`. Re-seeded on 2026-05-23 after the deterministic
label-aware declutter pass (`layout::declutter`) landed and the gentle
`sprawl = 0.1` compactness counterweight was added to `MetricWeights::default()`
-- both change the recorded sample costs. Do not seed the full metasd corpus
here: that is minutes-scale and produces a large JSON.
