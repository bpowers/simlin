...
/// OMITTED from the element map and `compiler::expand_per_element`
...
/// Its growth loop `c -> b -> c` is reinforcing for 146 active steps and
/// balancing for 53 once the alcohol level pushes `1.1 - 0.1*a` negative,
/// so it is `Undetermined` on either base; on the relative base the
/// confidence is 0.9655 (r = 99.9, |b| = 1.76 over the partition-relative
/// shares), the reading a dominance-weighted time share gives it.
#[test]
fn yeast_growth_loop_stays_undetermined_on_the_relative_base() {
    ...
    let positive = series.iter().filter(|v| **v > 0.0).count();
    let negative = series.iter().filter(|v| **v < 0.0).count();
    assert_eq!((positive, negative), (146, 53));
    assert_eq!(
        growth.polarity,
        DetectedLoopPolarity::Undetermined,
        "got {:?} at confidence {}",
    ...
    assert!(
        (growth.polarity_confidence - 0.9655).abs() < 1e-3
            && growth.polarity_confidence < POLARITY_CONFIDENCE_THRESHOLD,
        "the relative-base confidence is 0.9655; got {}",
        growth.polarity_confidence
    );
}
