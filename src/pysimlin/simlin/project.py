...
def _coerce_dt(value: Any) -> project_io.Dt:
    """Coerce input into a Dt protobuf message."""
    if isinstance(value, project_io.Dt):
        dt = project_io.Dt()
        dt.CopyFrom(value)
        return dt
...
class Project:
    ...
    def set_sim_specs(self, **kwargs: Any) -> None:
        ...
        specs = project_io.SimSpecs()
        fields = dict(kwargs)

        sim_specs_msg = fields.pop("sim_specs", None)
        if sim_specs_msg is not None:
            if not isinstance(sim_specs_msg, project_io.SimSpecs):
                raise TypeError("sim_specs must be a project_io.SimSpecs message")
