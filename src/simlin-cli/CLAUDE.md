...
A diagnostic that points at a span is printed WITH that span: `collect_formatted_errors` renders the offending equation and underlines `start..end` beneath it, so `1 +` prints its own text above `error in model 'main' variable 'bad': unrecognized_eof`. This is not cosmetic -- a parse error's whole reason IS the text under its span (the raising site writes no sentence, see [simlin-engine's CLAUDE.md](/src/simlin-engine/CLAUDE.md)), so the snippet-free formatter leaves parse errors as the one class of failure the CLI cannot explain.
