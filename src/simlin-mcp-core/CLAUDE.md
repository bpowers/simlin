...
- `src/test_support.rs` -- `#[doc(hidden)]` integration-test fixtures, gated behind the `test-support` feature so they are not compiled into shipped binaries (the crate takes a self dev-dependency enabling the feature so `tests/` still resolves them). `TestFileSystemAccess` is now an alias for the production `fs_access::FileSystemAccess`, not a second implementation; `chain_scc_project_json` builds the oversized-SCC model the LTM auto-flip warning tests need.
