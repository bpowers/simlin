...
// aligned at offset 24 = 32 bytes total. `polarity_confidence` grew the struct
// additively past its old 20-byte size (GH #495); validateStructSizes asserts
// this against simlin_sizeof_loop() so any miscalculation fails loudly.
const LOOP_SIZE = 32;
...
export function readLoops(loopsPtr: SimlinLoopsPtr): Loop[] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    loops.push({ id, variables, polarity, name, polarityConfidence });
  }
