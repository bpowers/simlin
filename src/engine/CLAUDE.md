...
- `tests/wasm-browser.test.ts` -- The browser flavor's source resolution: no source adopts the bundler-instantiated artifact (stubbed via an `rstest.config.mts` alias on the `.wasm` import, `tests/stubs/browser-wasm-stub.ts`), while bytes / module / URL / provider / `configureWasm` instantiate the caller's source and never touch the bundle
