...
- `healthz.ts` -- Unauthenticated health GET route for uptime checks, mounted under the API prefix (GET api/healthz) because a bare top-level healthz path is GFE-reserved on App Engine and never reaches the app. 200 when the WASM engine is ready; never touches Firestore. A preload failure aborts boot before the route mounts, so a broken instance surfaces as a connection failure, not a 503 -- the 503 branch is defense-in-depth
