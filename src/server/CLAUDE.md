...
- `seshcookie/` -- Vendored copy of the seshcookie encrypted-cookie-session library (github.com/bpowers/seshcookie-js; source, tests, and LICENSE). simlin is its only consumer, so it lives in-tree instead of going through npm publishes; keep diffs against upstream minimal
