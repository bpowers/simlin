...
- **`src/project.rs`** - Load, reference-count, and query projects:
  ...
  - `simlin_project_get_errors()` - Collect all diagnostics (static analysis + compile + VM-validation). When the project's `ltm_requested` latch is set (any prior `simlin_sim_new` with `enable_ltm=true`), it transiently re-enables the salsa `ltm_enabled` input via the shared `analysis::LtmEnabledGuard` for the duration of diagnostic collection, so the LTM diagnostic pipeline (auto-flip-to-discovery warning, synthetic-fragment compile failures, the GH #311 partial-equation warnings, the GH #486 non-Euler Error) reaches the caller -- the gap GH #466 closed. `simlin_sim_new` resets `ltm_enabled` to false right after compile (to avoid leaking into patch validation), so without the latch+re-enable the LTM gate in `model_all_diagnostics` left every LTM diagnostic invisible here. The toggle happens under the db lock, the same lock `simlin_sim_new` holds for its own LTM flag dance, so the two serialize and a concurrent sim creation never observes a partial LTM state; salsa memoizes the LTM synthesis from the `sim_new` compile, so the re-enable revalidates rather than recomputes. A project that never requested LTM skips the toggle entirely and pays no LTM synthesis cost (preserving the scoping the engine's `test_ltm_disabled_gate_suppresses_auto_flip_warning` pins). The `ltm_requested` latch is a set-once `AtomicBool` on `SimlinProject`; `LtmEnabledGuard` (in `src/analysis.rs`, `pub(crate)`, also used by the from-wasm rel-loop FFI) restores the flag unconditionally on drop, even on panic
