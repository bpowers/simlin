...
/// AC7.2: simlin_project_diagram_sync uses incremental layout when
/// patch_json is provided and the model already has a non-empty view.
///
/// Verifies the full end-to-end path: generate initial layout, apply a
/// patch adding a new variable, call diagram_sync with the patch JSON,
/// and confirm existing element positions are preserved while the new
/// variable gets a view element.
#[test]
fn test_ac7_2_incremental_layout_via_patch_json() {
    ...
    unsafe {
        ...
        // 1. Generate initial full layout (no patch)
        ...
        assert!(
            initial_positions.len() >= 3,
            "should have positions for at least population, births, rate"
        );

        // 2. Apply a patch that adds a new auxiliary "extra"
        let patch_json = r#"{
            "models": [{
                "name": "main",
                "ops": [{
        ...
        // 3. Call diagram_sync WITH the patch JSON (incremental path)
        ...
        // 4. Verify: existing positions preserved and new element present
        {
            let dm = (*proj).datamodel.lock().unwrap();
            let model = dm.get_model("main").unwrap();
            ...
            match &model.views[0] {
                engine::datamodel::View::StockFlow(sf) => {
                    // The new variable must have a view element
                    let has_extra = sf.elements.iter().any(|e| {
                        e.get_name().is_some_and(|n| n == "extra")
                    });
                    ...
                    for (name, orig_x, orig_y) in &initial_positions {
                        let found = sf.elements.iter().find(|e| {
                            e.get_name().is_some_and(|n| n == name)
                        });
