...
- `retainedLoops`: How many loops cleared the importance filter before the 200-loop report cap truncated `loopDominance`. When this exceeds the length of `loopDominance`, you are looking at a coverage-aware subset of the retained loops -- each step's dominant loop per feedback partition is guaranteed a slot and the rest is filled by mean importance -- presented in importance order but not a strict most-important-first prefix, so an omitted loop can outrank a reported one.
