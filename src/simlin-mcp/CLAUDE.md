...
- `src/lib.rs` -- Library half. `pub mod access` is now a one-line re-export of `simlin_mcp_core::fs_access::FileSystemAccess`, keeping the historical `simlin_mcp::access::FileSystemAccess` path working for integration tests that exercise it without spawning the binary. The impl moved to `simlin-mcp-core` (see that crate's `fs_access.rs`) so its integration suites stop running against a divergent copy; behaviour is unchanged -- `open` uses `tokio::fs::read_to_string` + `open_project`, `save` serialises per `SourceFormat` and atomic-writes, `create` validates non-existence then atomic-writes, `.mdl` writes are rejected with the verbatim canonical message, and `SourceFormat::SdaiJson` regenerates `relationships` from `compute_link_polarities` + `generate_relationships`.
...
- `tests/integration/file_system_access.rs` -- E2E coverage of `FileSystemAccess` (open/save/create, `.mdl` rejection, and the SD-AI `relationships` regeneration on save). It stays in this crate even though the impl now lives in `simlin-mcp-core`: these are the tests of the binary's chosen backing store.
