...
- **Post-commit work runs in `componentDidUpdate` / setState callbacks, not `setTimeout(0)`**: `onSelectionChanged` (above) and the cached-error refresh after navigation are driven from `componentDidUpdate`. The error refresh triggers on `prevState.modelName !== this.state.modelName` (covers drill-in, navigate-back, navigate-to-level). Viewport restoration in `handleNavigateBack`/`handleNavigateToLevel` runs in the `setState` completion callback so `getView()` resolves to the just-restored model. These post-commit hooks never fire after unmount, so unlike the genuinely-async timers (`openInitialProjectTimer`, `simRunTimer`, `saveTimer`, `undoRedoTimer`), they need no `unmounted` guard or `clearTimeout`.
