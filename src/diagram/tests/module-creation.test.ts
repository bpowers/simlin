...
// -- Module creation at any depth (AC5.2) --

describe('module creation targets current model via modelStack', () => {
  it('uses the current modelName for the patch, not hardcoded main', () => {
    // When navigated into a child model, state.modelName reflects
    // that child model. The patch is constructed using this.state.modelName,
    // so modules created inside a child model target the correct model.
    const modelStack = [{ modelName: 'main', moduleName: 'hare_population' }];
    const currentModelName = 'hares';

    // Simulate the patch construction from Editor.handleCreateVariable
    const patch = {
      models: [
        {
    ...
    // The patch targets the inner model, not 'main'
    expect(patch.models[0].name).toBe('hares');
    expect(patch.models[0].name).not.toBe('main');
    // The modelStack confirms we're nested
    expect(modelStack.length).toBeGreaterThan(0);
  });

  it('targets main model when modelStack is empty', () => {
    const modelStack: Array<{ modelName: string; moduleName: string }> = [];
    const currentModelName = 'main';
    ...
    expect(patch.models[0].name).toBe('main');
    expect(modelStack.length).toBe(0);
  });
});
