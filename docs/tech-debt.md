...
- **Status (2026-05-09)**: candidate implemented on branch `deploy-staging-bundle` -- `scripts/deploy-web-bundle.sh` (`pnpm deploy:web:bundle`). It runs `pnpm --filter=@simlin/server deploy --legacy --prod .deploy-bundle`, then vendors `@simlin/{core,engine}` out of `node_modules` and rewrites their `workspace:*` specifiers (and `@simlin/core`'s transitive `@simlin/engine` ref) to `file:` refs against `.deploy-bundle/vendor/`, drops `devDependencies`, regenerates a matching `pnpm-lock.yaml` via `pnpm install --lockfile-only --ignore-workspace`, copies in `public/` (assembled) / `config/` / `default_projects/` / `.app.prod.yaml` (as `app.yaml`), and `gcloud app deploy`s from the bundle (with a `trap` that restores the `public/` symlinks + tracked `index.html` and removes `.deploy-bundle` on exit/Ctrl-C). Resolves the `workspace:*` blocker that was the prior reason this wasn't done.
- **Validated locally**: the bundle is ~100 MB vs ~800 MB for the full workspace `node_modules`; its top-level `node_modules` has exactly the ~17 right packages (no `firebase-tools`/`@rsbuild`/`jest`); `pnpm install --prod --frozen-lockfile --ignore-workspace` in a copy of the bundle (standing in for GAE's instance-side re-install) completes in ~200 ms as a no-op; `node lib/index.js` from the bundle boots cleanly -- `WASM module initialized successfully`, Express listening, no `Cannot find module`.
- **Remaining**: a real `gcloud app deploy --no-promote` from the bundle to confirm GAE's Node buildpack accepts it and the instance starts; if it does, fold `deploy:web:bundle` into `deploy:web` and close this item. `pnpm deploy:web` (the monorepo-root flow) is unchanged on `main` as the fallback meanwhile.
