...
- **Description**: This entry used to claim that the `errors`/`unit_errors` fields on `Variable` and the `errors` field on `ModelStage0`/`ModelStage1` were all "redundant with the salsa incremental compilation pipeline ... dead weight carried through the monolithic compilation path", and that removing them would simplify the data model. Every read site was classified before acting, and the four fields fall into two very different groups.
  ...
  **`Variable::diagnostics` is a LIVE error channel, not residue.** It is how parsing and lowering report a failure on a variable (as context-free `DiagnosticError`s), and the salsa diagnostics are *downstream* of it: `db::var_fragment::explicit_fragment_input` reads the parsed variable's `Unit` entries (the malformed-`<units>`-string rows), its fatal entries (parse errors, where the conveyor/queue driven-flow `EmptyEquation` suppression applies) and the lowered variable's fatal entries (`MismatchedDimensions` and everything else `lower_ast` raises), and turns each into a `Diagnostic`. Emptying the lowered read makes every dimension-mismatch diagnostic vanish; emptying the parsed read turns a spec-sanctioned empty equation on a conveyor-driven flow into a phantom error. Deleting the field without first replacing the channel would silently drop diagnostics. The claim is pinned as a test rather than left as prose: `db::diagnostic_tests::variable_error_fields_are_the_lowering_channel` asserts both that the stage's value carries the error in the field and that the matching diagnostic reaches `collect_all_diagnostics`.

  **No whole-model lowered copy carries an error list.** The duplicate-canonical-ident collision (GH #891) is derived once per model by `db::diagnostic::model_duplicate_variables` and reported by `emit_duplicate_variable_diagnostics` (and as the hard error in `compile_project_incremental`); there is no whole-model simulatability gate reading a stage-level error list: `compile_project_incremental` is the only compiler, and its cycle gate (`db/dep_graph.rs`) decides whether a resolved recurrence SCC compiles.
...
- **Test impact**: `tests/integration/main.rs` compiles `mod smoke` out on Windows, so `ci.yaml`'s `serve-smoke` matrix runs the smoke test on ubuntu and macOS only, and Windows coverage of simlin-serve is the `serve-windows-check` job's `cargo check -p simlin-serve --all-targets`: a regression that breaks the Windows compile still trips CI, a Windows runtime regression does not. Resolving this item means dropping the cfg gate and adding windows-latest back to the `serve-smoke` matrix.
- **Investigation hints**:
...
### 60. No CI feature matrix for `simlin-engine`'s lib-only configurations
...
- **Description**: Neither the pre-commit hook nor CI exercises a Cargo **feature matrix** for `simlin-engine`, so feature-gating bugs in the **library** are invisible to the canonical gate. simlin-engine features: `file_io`, `schema`, `ai_info`, `debug-derive` (default = `schema, ai_info, debug-derive`), plus `xmutil`, `ext_data`, `png_render`, `ltm_bench`.
  ...
  **What remains.** The gap narrows to **lib-only** configurations: nothing gates `cargo check -p simlin-engine --lib --no-default-features` (or other non-default lib feature subsets), where a `#[cfg(feature = ...)]` mistake in `src/` can still break the build. In practice this is caught indirectly by the pre-commit WASM build (src/engine/build.sh's `cargo rustc -p simlin --lib --release --target wasm32-unknown-unknown --crate-type cdylib --no-default-features`, which compiles the engine with `file_io`/`schema`/`png_render` off), but that is one point sample, not a matrix, and it is a *release* build of a different crate. Note also that the self dev-dep means `--all-targets` no longer type-checks the lib with `file_io` off — so a matrix step must be `--lib`-scoped to test anything.
- **Suspected fix**: Add a lib-scoped feature-matrix check to the pre-commit hook and/or CI: at minimum `cargo check -p simlin-engine --lib --no-default-features` and `cargo check -p simlin-engine --lib --all-features`. Optionally `cargo hack --feature-powerset --lib` (bounded). Do **not** use `--all-targets --no-default-features`: the crate's own unit tests require `debug-derive` (thousands of `assert_eq!`/`{:?}` sites) and have never compiled without it, so that invocation fails for reasons unrelated to feature-gating hygiene. Keep within the documented test-time budget ([docs/dev/rust.md#test-time-budgets](dev/rust.md)) — a couple of representative lib configurations, not the full powerset, is the pragmatic choice given the 3-minute workspace cap.
- **Secondary note** (workflow, not a separate item): the original break propagated because multi-agent plan execution structurally trusts the inherited baseline — each subagent verified only its own delta, and several built *with* `file_io` and concluded the base was clean. Subagents should verify the exact configuration they will later claim green BEFORE starting, and STOP if the inherited baseline is already broken.
- **Owner**: unassigned
