...
- **Note (2026-06-09):** The y-delta-vs-slope residual (GH #536) is now resolved -- `analyze_graphical_function_polarity` classifies each segment by its slope `dy/dx` against an average-slope-relative tolerance (`max(1e-12, 1e-6 * (y_max - y_min) / (x_max - x_min))`), with a degenerate `x[i] == x[i-1]` segment skipped (duplicate point) or bailed to `Unknown` (genuine vertical step). On uniform x-spacing this reduces to the prior y-delta behavior, so #492's import-noise tolerance is preserved.
...
- **Why it's not fixed yet**: The clean fix is a self-contained server bundle deployed from a staging directory: `pnpm --filter @simlin/server deploy --legacy --prod <dir>` produces a `<dir>/node_modules` with only the server's prod deps materialized (~100 MB instead of ~GB-scale, no `@rsbuild`/`jest`), and `<dir>` would also contain the assembled `public/`, `config/`, `default_projects/`, `lib/`, and `app.yaml`. The blocker for shipping this: the deployed `<dir>/package.json` still carries `"@simlin/core": "workspace:*"` and `"@simlin/engine": "workspace:*"`, and GAE's buildpack always re-installs from `<dir>/package.json` on the instance regardless of any uploaded `node_modules` -- workspace protocol refs can't be resolved outside a workspace, so `npm/pnpm install` would fail. Resolving that needs (a) post-processing `<dir>/package.json` to vendor the `@simlin/*` packages as `file:./vendor/...` references with rewritten transitive workspace refs, plus (b) a real `gcloud app deploy --no-promote` test against a staging GAE version to confirm the buildpack actually accepts the result. Neither has been done.
- **Verified properties of the staging-dir bundle** (as of 2026-05-08, when the server's prod deps still included `@google-cloud/trace-agent`, `body-parser`, `passport`, `passport-strategy`, `serve-favicon`, `uuid`, and `winston` -- since removed): `pnpm deploy --legacy --prod` produces a bundle whose top-level `node_modules` has exactly the server's prod-dep set (no `@rsbuild`, no `jest`); every prod dep resolves correctly; `node_modules/@simlin/engine/core/libsimlin.wasm` is materialized; `require('./lib/app.js')` and `require('@simlin/engine')` both load. The transitive `.pnpm` virtual store is ~100 MB of legitimate Google Cloud SDK + firebase-admin closure.
- **Measure**: `du -sh node_modules/` on a fresh GAE instance; also `ls node_modules/.pnpm/ | wc -l` on the instance vs the ~340-package closure a `pnpm deploy --prod` produces.
- **Owner**: unassigned
