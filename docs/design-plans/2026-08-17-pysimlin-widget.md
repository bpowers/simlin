...
1. `pip install pysimlin` (no sidecar process; the interactive editor needs the `notebook` extra) gives `simlin.open(path)` -> a file-backed model that knows its path, saves in the same format it was loaded from (`.stmx`/`.xmile`, `.mdl`, native `.sd.json`), tracks a revision, and reloads when the file changes on disk (so Claude Code editing the file, or the `simlin` MCP server, is picked up).
...
Decisions locked with the user on 2026-08-17: the file on disk is the authority when kernel and widget disagree; the primary Claude participation path is Claude Code running cells / editing the model file in the same repo (MCP must also work but is not the first-class channel); Colab is a target, so no local sidecar server may be required; native JSON, `.stmx`/`.xmile` and `.mdl` are all first-class on-disk formats and `.mdl` is written in place (the engine's Vensim writer exists; no sidecar); the widget assets ship inside the pysimlin wheel; `anywidget` is an optional extra (`pip install "pysimlin[notebook]"`) rather than a hard dependency (decision revised 2026-08-18: its chain is ~22 packages / ~56 MB, paid mostly by MCP-server and script installs that never display anything; a bare install shows the SVG diagram plus a one-line hint).
...
- Widget -> kernel: `{type:'wasm'}`; `{type:'snapshot', id:string, base:int, json:string}` -- the whole snapshot rides in the message (custom messages are ordered and never merged); `id` is the request id, unique per view and per request (a token minted once per mounted view plus a sequence number).
...
- Kernel -> widget: `{type:'wasm'}` + first binary buffer (or `{type:'wasm', error}`); `{type:'saved', revision, id}`; `{type:'rejected', revision, id}` -- `id` is the snapshot's `id` echoed verbatim (any JSON value; omitted only when the snapshot carried none) from every arm, including a malformed snapshot's rejection; `{type:'notice', text, level?}`. `oversize` carries no id and is owed no reply.
- ONE snapshot in flight per view, replies matched by `id`: the Editor's `onSave` sends `{type:'snapshot', id, base, json}` and resolves only when the reply echoing that `id` arrives (`saved` -> new version; `rejected` -> `undefined`, remount from the authoritative traits). A reply naming another request, or none, is ignored whatever is in flight -- another view's reply (every custom message reaches every view of one model), the reply owed to a snapshot whose slot a remount freed, a reply after unmount -- so the kernel must echo the id. A reply with a matching `id` but a missing or non-integer revision is treated as a reject. `ProjectController` already serialises saves (one in flight plus one queued flush that re-reads the server version), so with a busy kernel the Editor keeps working locally and a single flush of the latest state goes when the reply lands. No timeout while in flight (a long-running cell legitimately delays the reply); the promise resolves `undefined` on unmount.
- Kernel accept: `_apply_snapshot(json, base)` succeeds -> file written -> inside one `hold_sync()` assign `project_json = <exact bytes received>` and `revision += 1` (plain assignments; the frontend never wrote this trait, so there is no property lock) -> send `{type:'saved', revision}`. Kernel reject (stale base, unparsable snapshot, or a handler failure before anything was applied): traits untouched, send `{type:'rejected', revision}` plus a notice. A snapshot that was APPLIED but whose file write failed is an accept (the kernel's revision has advanced and the project is `dirty`): push the exact bytes and `revision + 1`, send `saved`, plus a warning notice telling the user to call `save()` -- replying `rejected` there would leave the browser's base behind the kernel forever. Every `snapshot` gets exactly one reply no matter what: the handler is wrapped so any exception ends in exactly one reply -- `rejected` plus a notice when nothing was applied (bad JSON, engine error, a failure before apply), `saved` plus a warning notice when the change was applied and only the write or the reply path failed -- because a missing reply leaves the widget's save queue waiting forever; reply `revision` values are JSON integers; the `hold_sync` block exits (state message sent) before `saved` is sent, and the widget tolerates the reverse order by adopting the acknowledged pair from the reply. Kernel-originated changes (`edit()`, disk reload): assign both traits in one `hold_sync()` (plus an "Updated on disk" notice for external sources).
- Widget on trait change (idempotent over the final `(revision, project_json)` pair): if a snapshot is in flight and the pair equals `(expected revision, in-flight json)` it is `own-ack` -- the live Editor is kept, but the seed is adopted only by the matching `saved` (the pair proves the kernel holds our bytes at base+1, not that our snapshot won: another view can send byte-identical bytes against the same base, after which our reply is `rejected` and must remount); otherwise remount `<Editor>` seeded from the pair (remount is idempotent on the pair, so a `rejected` reply following a disk push does not remount twice). Own accepted saves therefore never remount and undo history survives; kernel/disk changes remount (undo history resets, as in simlin-serve).
...
- Dependency footprint: owning the widget stack without anywidget/ipywidgets was spiked and works (a comm-only Python widget over `comm` plus a 21 KB federated labextension built with rspack's Module Federation, proven in JupyterLab 4.6 and Notebook 7.6 with only `jupyterlab_widgets` present), but VS Code's Jupyter extension only starts its widget renderer when `import ipywidgets` succeeds in the kernel and marimo only renders `anywidget.AnyWidget` instances, so owning the stack would lose two named hosts to save two packages over the optional extra. The extra is the standing choice; the spike is the fallback if the host list ever shrinks to JupyterLab/Notebook 7.
