...
class ModelWidget(anywidget.AnyWidget):
    ...
    max_snapshot_bytes = Int(8 * 1024 * 1024)  # kernel-owned cap, see below
```
...
- Widget -> kernel: `{type:'oversize', bytes:int}` -- sent INSTEAD of a snapshot whose UTF-8 size exceeds the kernel-owned `max_snapshot_bytes` trait (default `MAX_SNAPSHOT_BYTES` = 8 MiB, identical in `_widget_core.py` and `widget-core.ts`); it is owed no `saved`/`rejected` reply -- the widget resolves that save `undefined` itself, keeps the local edit, and shows the warn toast "Edit not saved: the model is too large for the notebook connection (N MiB > M MiB limit); edit it from Python instead." The kernel answers with a `RuntimeWarning` in the cell output plus the same warn notice (identical text, so the two collapse), and warns once per widget when it seeds or pushes a `project_json` above the cap. 8 MiB, not 10: the snapshot rides JSON-escaped inside the comm envelope, 7-16% larger on the repo's models, so 8 MiB is about 9.3 MiB on the wire, inside tornado's 10 MiB default with headroom; users who raise `--ServerApp.tornado_settings` raise `max_snapshot_bytes` with it.
