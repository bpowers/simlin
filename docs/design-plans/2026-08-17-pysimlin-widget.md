...
- `simlin_project_serialize_mdl(project, out_buf, out_len, out_err)` -- exposes `simlin_engine::to_mdl_with_warnings`; warnings are surfaced through the collected-errors channel (not fatal).
- `simlin_project_replace_from_json(project, data, len, format, out_err)` -- replaces the `datamodel::Project` inside an existing `SimlinProject` (under its mutex) and re-syncs the salsa db, so `SimlinModel` handles (which hold `*const SimlinProject` + name) stay valid across a reload. Rejects (leaves the project untouched) if the JSON fails to parse. Also accept protobuf/XMILE/MDL bytes via the same entry (a `format` enum) so pysimlin's reload does not need to convert on the Python side.
...
3. Claude Code / MCP / `git checkout` writes the file -> poll thread sees mtime change -> read bytes -> hash != last_written -> parse in-place (`_replace_from_bytes`) -> if parse fails: warn, keep last-known-good, remember bad hash so we don't re-warn every poll -> else `revision += 1`, `on_change(disk)`, traits pushed with `notice="Updated on disk"`.
...
**Phase 1 -- engine + pysimlin foundations.** FFI: `serialize_mdl`, `replace_from_json` (multi-format). pysimlin: format table, `open()`, `Project.path/format/revision/dirty/autosave/save/save_as/reload`, `_replace_from_bytes`, incremental `diagram_sync(patch)` in `edit()`, `updateStockFlows` op parity, `_sync.py` state machine, poll-thread watcher, `on_change`, `Model.diagram()` + `_repr_svg_`, README/CLAUDE.md docs. TDD throughout; round-trip tests per format (including `.mdl` sketch preservation).
