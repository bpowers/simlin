...
`model_stage1`, and finish the architecture current-state documentation. The
production readers are `check_model_units`' reachable `ModelStage1` map,
`check_conveyor_param_units`' augmented Stage0 lowering, and the stage module's
own module-scope construction; `units_infer::infer_project` and the stage,
diagnostic, dimension-invalidation and PREVIOUS/INIT comparisons are test
oracles. The remaining production `BTreeSet<String>` values are lookup holders
...
that rewrite and cannot return scheduling facts by type.
