...
source-parameterized collection; explicit and ordinary implicit module
discovery feeds typed candidates to one target-validation, instance-key
insertion and recursive-descent owner; generated LTM equations consume the
post-module-expansion AST and therefore add capture helpers, not module
instances; one bound-port predicate filters internal sources and foreign
destinations for both emitted module inputs and compilation identity),
`common.rs` (`EquationError.details`), `db/fragment_compile.rs`
...
**8b -- compiler-expression facts.** A compiled explicit fragment carries one
`flow_locally_invariant` boolean from the exhaustive compiler-Expr walk with
every `VarRef` treated as invariant. That walk owns only facts introduced or
made precise by lowering -- temps, module evaluation/input loads, constant
folding, and `BuiltinSig::invariance`. `db/invariance.rs` combines it with the
authoritative source relation:
`local compiler invariant && every local Dt/Current DepRef target invariant`.
`Dt/Initial` reads the frozen buffer and `Dt/Previous` is locally lagged. The
source relation deliberately retains both eager conditional arms for
validation, runlists and cycles; a dynamic target in a constant-discarded arm
causes a safe under-hoist rather than polluting `DepRef` with optimizer
reachability. `collect_expr_refs` and fragment `dep_names` do not exist.

LTM's syntax-only freeze and pin membership is
`variable::expression_transform_names -> ExpressionTransformNames`; the
crate-private result can carry only value rewrite candidates and lookup-table
holders, never phase, lag, a resolved module path or a target. Every scheduling
consumer reads `DepRef`. The remaining LTM call to `classify_dependencies` is
`lower_ltm_variable`'s generated-equation shape construction: it immediately
resolves occurrences to `DepTarget` and uses only local structured targets plus
the textual lookup layout channel to recover array dimensions. Exhaustive and
discovery module exits read the same `CausalGraph` projection of complete dt
`DepTarget`s for explicit and parse-synthesized readers. Nested paths, equal
leaves below distinct roots and multi-output ambiguity remain structural; no
module-prefix stripping or reconstructed-AST identity scan exists at that
boundary. The only production `dep_head` callsites consume
`referenced_tables`, the intentionally textual lookup-table layout channel.
`analysis.rs` edge maps and runlist/result sets are presentation names, not
source dependency relations. A resolved compiler `VarRef` or physical offset
cannot replace scheduling identity: resolution collapses the first instance to
a layout base plus terminal element offset and therefore loses nested instance
names, the terminal source ident, and the pre-assembly `(model, input-set)` key
required by sparse initials and diagnostics.

The exact C-LEARN invariant-slot diff against `4d5302d6` has no removals and
fifteen additions. Offsets 902--908 are the seven COP elements of
`forestry_emissions_at_start_year`; their graphical-function table is an
immutable layout holder and their index reads only constant
`forestry_start_year` and `one_year`. Offsets 2371--2377 are the seven COP
elements of `projected_population_in_target_year`; their three UN population
tables are immutable layout holders and all value inputs are invariant: the
constant scale, scenario, scenario weights and `one_year`, plus the INIT-frozen
target year. Offset 2370 is the pure SUM
`projected_global_population_in_target_year`. A production-derived
lookup/reducer matrix proves both decision classes and their TIME-indexed
negative controls through source extraction, lowering, final offsets, and
exact VM series.

**8c -- whole-model stages and final docs.**
`lowered_source_variable` and `lowered_implicit_variable` own one shared
`Arc<Variable<Expr2>>` payload per variable. Fragment compilation borrows that
handle; unit checking and the model-wide causal/polarity/LTM maps clone only the
Arc, with pointer identity pinned against the production memos. Per-name LTM
lookups bypass the model-wide map. A stack-local `UnitModel` supplies the graph
unit inference needs and cannot cross a query boundary. Conveyor parameters use
a transient parsed/lowered scope containing memo-owned source handles plus only
their generated parameter auxiliaries. `model_module_targets` projects
explicit instances and parse-synthesized module targets from the production
parse metadata; `model_scope_models` closes those lightweight name edges
iteratively and safely across cycles. Neither topology query walks or owns an
equation tree.

The remaining production `BTreeSet<String>` values are lookup holders
(`VariableDeps`, implicit deps and LTM shape/pinning), dimensions (`sync` and
`variable_relevant_dimensions`), compiler-local shape candidate names already
projected from `DepTarget` (`var_fragment`), runlist/invariant name results, or
graph/result presentation. Test fixtures add no production owner. The only
...
artifact level (the genuine-output corpus, the fragment goldens, and the
wasm-vs-VM parity run). Unit checking borrows the production per-variable
lowering memos into a stack-local model graph; it owns no second equation tree.
...
2. A defensive generic-parser module-shaped LTM implicit helper's sub-variables
   are keyed `helper·sub`, the separator every other module instance uses,
   where the running-count flatten spelled them `helper.sub`. Production
   `parse_ltm_equation` inputs consume the post-module-expansion AST, and
   `model_ltm_implicit_var_info`'s rustdoc records that its production payload
   is PREVIOUS/INIT capture metadata only, so this generic shape is absent.
...
|---|---|---:|---:|---|---|---|
| G closure | `engine: centralize module instance traversal` | Against exact `8bc52697` and final release binaries pinned to performance core 2, five alternating whole-process compile-channel pairs with `CLEARN_COMPILE_ITERS=5` put plain at 7.35461 G instructions current versus 7.34874 G base by per-side medians (+0.0799%); the median paired delta is +0.0678% (range -0.0356%..+0.1553%). LTM is 81.40348 G versus 81.40658 G (-0.0038% by per-side medians); the median paired delta is -0.0106% (range -0.0165%..+0.0456%). Both are within the compile instruction channel's approximately 0.15% resolution floor; no compile-cost movement is established. | 5189 plain / 28490 LTM | plain 28637 / 1477 / 24711; LTM 830380 / 1477 / 24711 | plain 1358 / 162 / 28 / 725; LTM 13113 / 162 / 28 / 2334 | `ModuleInstanceCandidate::{Explicit, OrdinaryImplicit}` is exhaustive over the two production module namespaces; `record_module_instance` owns target validation, canonical model/input-set identity, insertion, deduplication, cycle termination and nested descent while each variant retains its exact missing-target diagnostic. The initial-dependency query delegates to that same top-level owner. Explicit candidates retain namespace precedence, and both namespaces sort their guaranteed canonical map keys before traversal; reversed-declaration and fresh-map multi-missing rows pin the exact first diagnostic. `bound_module_port` is the one policy behind emitted `ModuleInput`s and enumerated input-set identity: foreign destinations and own-prefix/internal sources bind nothing. A synced internal-reference row derives the empty enumeration key and emitted `EvalModule` declaration from production, resolves the same compiled child in the VM and reads its output. The remaining production-derived rows cover both candidate variants together with real LTM instrumentation, same-target distinct and duplicate port sets, nested discovery, both missing-target policies and cycles. Every `LtmEquation` producer consumes the post-module-expansion AST or combines already-generated qualified references; a zero-search for the six canonical module functions (`smth1`, `smth3`, `delay1`, `delay3`, `trend`, `npv`) and three source aliases (`delay`, `delayn`, `smthn`) finds no generated call. A source SMTH1 feedback row proves its module stays in `model_implicit_var_info` while production LTM parsing adds only PREVIOUS/INIT captures with no module or hoisted-argument metadata, so no LTM module candidate exists. Plain and LTM C-LEARN profiles are byte-identical to the base after removing timing and embedded source-path lines, including totals 54825 / 856568, post-fusion flow+stock 17017 / 384463, result offsets 4900 / 13800 and every slot/opcode/histogram/literal/GF/temp/dimension/view/name/module count. Release LTM user-series parity, every generated partial, the complete 23-warning surface and all-slot digest `(19264, 0, 3106, 19264, 790401758590, 2212, 7484877280482623718)` are green. Validation: engine lib 5760 passed / 2 ignored; integration 787 passed / 16 ignored; VM allocation 4 passed; doctests 2 passed / 1 ignored; fragment, diagnostic, module enumeration, exact multi-missing and internal-reference determinism pass 12 consecutive process invocations; all-target/all-feature engine clippy with warnings denied, rustfmt and `git diff --check` are green. |
