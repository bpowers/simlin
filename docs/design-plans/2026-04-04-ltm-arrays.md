...
- **A2A (Apply to All)**: An equation mode where a single equation template is evaluated independently for each element of a dimension. The compiler expands the template into per-element bytecode via `expand_per_element`. Represented as `Equation::ApplyToAll(dims, eqn)` in the AST.
...
The equation TEXT is unchanged. For A2A link scores, the same ceteris paribus formula applies per element via the compiler's existing `expand_per_element`. No new A2A expansion code is needed.
...
**A2A equation compilation** (`src/simlin-engine/src/compiler/mod.rs`): The compiler already expands `Ast::ApplyToAll(dims, expr)` into per-element bytecode via `expand_per_element`. The LTM pipeline change reuses this by producing `Equation::ApplyToAll` instead of `Equation::Scalar`, triggering the same expansion path.
