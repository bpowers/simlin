...
through one source-expression path for scalar, arrayed and apply-to-all
equations. The lookup-only rule is applied before the final
`compiler::array_operand` materialization pass, reusing the per-element table
layout for every equation shape.
...
- Uniform lookup-only lowering in `src/simlin-engine/src/compiler/mod.rs`:
  scalar, arrayed and apply-to-all expressions apply the per-element table
  under the same rule before post-resolution array materialization. Reuses
