...
- [notebooks/](../notebooks/) -- Notebook GENERATORS. Each `build_*.py` constructs an analysis notebook cell-by-cell and executes it against the current engine, with a `verify_*.py` that fails on cell errors, silent cells, missing figures, and missing claim markers; the executed `.ipynb` is a gitignored artifact, so an audit is always re-derived rather than read. `build_ltm_discovery_audit.py` (`--model clearn|wrld3`) audits LTM discovery: it dumps the engine's exact link-score edge set, re-enumerates the loop universe in pure Python, and diffs universe size, scores, ranking, and per-step dominance against what the engine reported
