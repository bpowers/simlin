...
function printPostDeploy(project, versionId, url) {
  ...
  console.log(
    [
      ...
      '  - In a browser: log in with EMAIL/PASSWORD, land on Home with no console errors.',
      '    (The domain/referrer authorization enables identitytoolkit calls from this',
      '    host. Google/Apple redirect login still CANNOT complete here: the SDK reads',
      '    the redirect result through an auth.simlin.com iframe, and browsers partition',
      '    cross-SITE iframe storage -- appspot.com vs simlin.com. It works on',
      '    app.simlin.com, which is same-site with the auth domain; verify OAuth there',
      '    right after promoting.)',
