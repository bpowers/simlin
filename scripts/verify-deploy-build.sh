...
#    ~17% of the full binary and shipping it to browsers is pure dead
...
#     unoptimized browser bundle is ~11% larger (9.5MB -> 10.6MB) and nothing
...
if [ "1" = "${REQUIRE_WASM_OPT-0}" ]; then
    for wasm in src/engine/core/libsimlin.wasm src/engine/core/libsimlin-browser.wasm; do
        if [ ! -f "$wasm" ]; then
            ...
            fail "$wasm was built WITHOUT wasm-opt (mode: $(cat "$wasm.mode")). Deploying it would ship a ~11% larger bundle. Is wasm-opt installed, and is DISABLE_WASM_OPT unset?"
